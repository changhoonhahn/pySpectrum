...
def fastPM(z, str_flag='', Lbox=205., Nmax=40, Ncut=3, step=3):     
    ...
    f_pell = ('%s/pySpec.Plk.%s.Lbox%.f.Ngrid360.dat' % (dir_fpm, f_halo, Lbox))
    f_pnkt = ('%s/pySpec.Plk.%s.Lbox%.f.Ngrid360.nbodykit.dat' % (dir_fpm, f_halo, Lbox))
    f_b123 = ('%s/pySpec.Bk.%s.Lbox%.f.Ngrid360.step%i.Ncut%i.Nmax%i.dat' % (dir_fpm, f_halo, Lbox, step, Ncut, Nmax))
...
def TNG(z, Lbox=205., Nmax=40, Ncut=3, step=3):     
    ''' calculate the powerspectrum and bispectrum of the fastPM catalog.
    '''
    dir_fpm = os.path.join(UT.dat_dir(), 'fastpm') 
...
def fastpm_v_tng(z, Lbox=205., Nmax=40, Ncut=3, step=3): 
    ''' make plots that compare the powerspectrum and bispectrum of 
    fastpm and illustris tng 
    '''
    dir_fpm = os.path.join(UT.dat_dir(), 'fastpm') 
    f_fpm = ('halocat_FastPM_40step_N250_IC500_B2_z%.2f%s.txt' % (z, str_flag))
    f_tng = ('halocat_TNG300Dark_z%.2f.txt' % z) 
    ...
    klim = (k < 0.3) 
    sub.plot(k[klim], p0k[klim], c='k', lw=1, label='FastPM') 
    k, p0k = np.loadtxt(f_pnkt(f_fpm), unpack=True, skiprows=1, usecols=[0,1]) 
    klim = (k < 0.3) 
    sub.plot(k[klim], p0k[klim], c='k', ls='--', lw=1) 
    k, p0k = np.loadtxt(f_pell(f_tng), unpack=True, skiprows=1, usecols=[0,1]) 
    klim = (k < 0.3) 
    sub.plot(k[klim], p0k[klim], c='C1', lw=1, label='Illustris TNG') 
    k, p0k = np.loadtxt(f_pnkt(f_tng), unpack=True, skiprows=1, usecols=[0,1]) 
    klim = (k < 0.3) 
    ...
    sub.set_xscale('log') 
    fig.savefig(os.path.join(dir_fpm, 'p0k_fpm_tng_z', bbox_inches='tight')
