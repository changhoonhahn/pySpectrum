...
def QPMvsAemulus(): 
    f_pell = lambda sim: ''.join([UT.dat_dir(), sim, '/pySpec.Plk.halo.mlim1e13', 
        '.Ngrid360', '.dat']) 
    f_pnkt = lambda sim: ''.join([UT.dat_dir(), sim, '/pySpec.Plk.halo.mlim1e13', 
        '.Ngrid360', '.nbodykit', '.dat']) 
    f_b123 = lambda sim: ''.join([UT.dat_dir(), sim, '/pySpec.B123.halo.mlim1e13', 
    ...
    sub.set_xlim([8e-3, 1.]) 
    sub.set_xscale('log') 
    fig.savefig(''.join([UT.dat_dir(), 'qpm/p0k_qpm_aemulus.png']), bbox_inches='tight')
    ...
    sub.set_xlabel('$k_1 > k_2 > k_3$ triangle indices', fontsize=25) 
    sub.set_xlim([0, len(q123)]) 
    fig.savefig(''.join([UT.dat_dir(), 'qpm/Q123_qpm_aemulus.png']), bbox_inches='tight')
    return None
